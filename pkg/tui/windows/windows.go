...
// KeyMap is the alias used inside this package for the windows keymap.
...
// Model is the exported windows TUI model. New() constructs one; Init/Update/View
// implement tea.Model.
type Model = windowsModel

// New constructs a Model. The host supplies the tmux client, the current session
// name, the keymap, and a flag controlling child-process detection.
func New(client *tmuxclient.Client, sessionName string, keys KeyMap, showChildProcesses bool) Model {
	return newWindowsModel(client, sessionName, keys, showChildProcesses)
}

// SelectedWindow returns the window the user picked, or nil if no selection
...
// Mode returns the current input mode (used by host UIs to know whether the
// user is in a text-input mode and should not propagate global keys).
func (m Model) Mode() string { return m.mode }

// SessionName returns the name of the tmux session this picker is browsing.
...
// LoadedMsg is emitted after the async window list fetch completes. Exported
// so host routers can identify it and forward it to this model even when it's
// not the currently-active view.
type LoadedMsg = windowsLoadedMsg

// PreviewLoadedMsg is emitted after the async pane-capture preview completes.
type PreviewLoadedMsg = previewLoadedMsg
...
func newWindowsModel(client *tmuxclient.Client, sessionName string, keys KeyMap, showChildProcesses bool) windowsModel {
	...
	case key.Matches(msg, m.keys.MoveMode), msg.Type == tea.KeyEsc, msg.Type == tea.KeyEnter:
		...
		if len(m.originalWindows) > 0 {
			...
			for i, win := range m.filteredWindows {
				...
				cmd := tmuxclient.Command("move-window", "-s", srcTarget, "-t", tempTarget)
				cmd.Run() // Best effort
			}
			...
			for i := 0; i < len(m.filteredWindows); i++ {
				...
				// Use tmux.Command to respect GROVE_TMUX_SOCKET
				cmd := tmuxclient.Command("move-window", "-s", srcTarget, "-t", finalTarget)
				cmd.Run() // Best effort
			}
